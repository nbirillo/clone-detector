...
class SuffixTreeComplexTest {
    private var sequencesAmount = 1000
    private var sequencesLength = 200
    private var sequencesRemovals = 800

    fun estimateMemory(amount: Int, length: Int) {
        createTree(amount, length)
        System.gc()
        val memory = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()
        println(memory / amount / length)
    }

    private fun createTree(amount: Int, length: Int): SuffixTree<LongToken> {
    ...
    fun testComplexRemoveSequence() {
        ...
        val removedIds = treeSequences.shuffle().take(sequencesRemovals).map { (id, _) -> id }.toSet()
        ...
        val remained = treeSequences.filter { (id, _) -> id !in removedIds }.map { (_, sequence) -> sequence }
        print("Check other existance...")
        checkSequences(tree, remained)
        println("OK")

        val removed = treeSequences.filter { (id, _) -> id in removedIds }.map { (_, sequence) -> sequence }
