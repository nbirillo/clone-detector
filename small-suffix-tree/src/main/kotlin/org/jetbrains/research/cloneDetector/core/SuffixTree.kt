...
class SuffixTree<T : Comparable<T>> {
    ...
    private fun update(s: Node, sequence: List<Any>, k: Int, i: Int): Pair<Node, Int> {
        var s = s
        var k = k
        var oldr = root
    ...
    private fun updateSequence(sequence: List<Any>) {
        var s = root
        var k = 0
        var i = -1
    ...
    private fun relabelAllParents(edge: Edge?) {
        var edge = edge
    ...
    private fun removeEdge(s: Node, sequence: List<Any>, k: Int): Pair<Node?, Int> {
    ...
    fun addSequence(sequence: List<T>): Long {
        if (sequence.size + 1 > Short.MAX_VALUE) {
            throw IllegalArgumentException("Sequence length should be less then ${Short.MAX_VALUE}")
        }
    ...
    fun getAllLastSequenceNodes(id: Long): List<Node> {
        ...
        for (i in 0 until sequence.size - 1) { // drop end token
