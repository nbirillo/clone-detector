...
internal fun <K, V> ScapeGoatTree<K, V>.put(element: ScapeGoatNode<K, V>): Stack<ScapeGoatNode<K, V>> {
    ...
    return if (root == null) {
        ...
        while (current !== element) {
            ...
            current = if (cmp < 0) current.getOrSetLeft(element)
            else if (cmp > 0) current.getOrSetRight(element)
...
internal fun <K, V> ScapeGoatTree<K, V>.remove(key: K) {
    delete(key)
...
internal fun <K, V> ScapeGoatTree<K, V>.get(key: K): ScapeGoatNode<K, V>? = root.search(key)

internal tailrec fun <K, V> ScapeGoatNode<K, V>?.search(key: K): ScapeGoatNode<K, V>? {
    if (this == null) return null
    val cmp = comparator.compare(key, this.key)
    return if (cmp > 0) right.search(key)
    else if (cmp < 0) left.search(key)
    else this
}

internal fun <K, V> findScapeGoat(
    parents: Stack<ScapeGoatNode<K, V>>,
...
internal tailrec fun <K, V> findMinimum(
    node: ScapeGoatNode<K, V>,
    nodeParent: ScapeGoatNode<K, V>?
): Pair<ScapeGoatNode<K, V>, ScapeGoatNode<K, V>?> {
    nodeParent?.checkIsChild(node)
    return if (node.left != null) {
        findMinimum(node.left!!, node)
    } else Pair(node, nodeParent)
}
...
internal fun <K, V> ScapeGoatTree<K, V>.replaceChild(
    parent: ScapeGoatNode<K, V>?,
    oldChild: ScapeGoatNode<K, V>,
    newChild: ScapeGoatNode<K, V>?
) {
    ...
    if (parent == null) {
        ...
        throw IllegalArgumentException("OldChild: ${oldChild.key} " +
                "New: ${newChild?.key} Parent: ${parent.key} ${parent.left?.key} ${parent.right?.key}")
    }
}
...
internal fun <K, V> ScapeGoatTree<K, V>.entries(): Collection<ScapeGoatNode<K, V>> = root.children(ArrayList())

internal fun <K, V> ScapeGoatNode<K, V>?.children(
    accumulator: MutableList<ScapeGoatNode<K, V>>
): Collection<ScapeGoatNode<K, V>> {
    if (this != null) {
