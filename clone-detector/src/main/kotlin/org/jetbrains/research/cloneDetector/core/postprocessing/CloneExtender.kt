...
fun sortedFileDuplicates(baseFile: VirtualFile, clones: List<CloneClass>): List<FileDuplicate> {
    return clones
        .map {
            FileDuplicate(it.clones.filter { it.file == baseFile }.minByOrNull { it.textRange.startOffset }!!, it)
        }
...
fun canUnite(first: Clone, second: Clone, distance: Int = PluginSettings.minFragment): Boolean {
    return if (first.file != second.file) false
    else second.firstPsi.hasLeftPsi(first.lastPsi, distance) || second.firstPsi.hasRightPsi(first.lastPsi, distance)
}
...
fun unite(first: Clone, second: Clone): Clone =
    ...
    } else RangeClone(second.firstPsi, first.lastPsi)
